...
def align_handler(img1, img2, pyramid_height, factor, freeze_x=False,freeze_y=False):
    ...
    for i in range(pyramid_height):
        displacement_size = int(factor*pyramid1[pyramid_height-1].shape[0]) if i == 0 else 2

        disp = align(pyramid1[i], pyramid2[i], displacement_size, freeze_y=freeze_y, freeze_x=freeze_x)
        dx += disp[0]*2
        dy += disp[1]*2
    print(dx,dy)
